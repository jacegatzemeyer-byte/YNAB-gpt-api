...
# Optional durable persistence. Railway Postgres injects DATABASE_URL when a
# PostgreSQL service is linked to this service. When absent, the middleware
# remains backwards-compatible with Railway environment-variable storage.
DATABASE_URL = (os.getenv("DATABASE_URL") or "").strip()
PLAID_HOUSEHOLD_ID = (os.getenv("PLAID_HOUSEHOLD_ID") or "household").strip()
if not PLAID_HOUSEHOLD_ID:
    raise RuntimeError("PLAID_HOUSEHOLD_ID must not be empty")
...
class BudgetStore:
    def __init__(self):
        ...
        # Hosted Link sessions keyed by a non-secret exchange reference (the
        # hosted-link URL slug, e.g. lp...). The real Plaid link_token remains
        # server-side and can also be persisted in Postgres.
        self.plaid_link_sessions: Dict[str, dict] = {}

        # Runtime mapping registry. Environment mappings are treated as seeds;
        # when DATABASE_URL is configured, database rows override them and
        # mapping changes persist without editing Railway variables.
        self.plaid_account_mappings: Dict[str, dict] = {
            str(plaid_id): {
                "plaid_account_id": str(plaid_id),
                "ynab_account": str(ynab_alias),
                "include_in_household": True,
                "source": "environment",
            }
            for plaid_id, ynab_alias in PLAID_YNAB_ACCOUNT_MAP.items()
        }
        self.persistence_ready: bool = False
...
# =====================================================================
# Durable Plaid Registry & Account-Mapping Persistence
# =====================================================================
...
def effective_plaid_mapping(plaid_account_id: str) -> Optional[dict]:
    """Return the active household mapping for one Plaid account, if any."""
    row = store.plaid_account_mappings.get(str(plaid_account_id))
    if not row or not bool(row.get("include_in_household", True)):
        return None
    ynab_account = str(row.get("ynab_account") or "").strip()
    if not ynab_account:
        return None
    return row


def included_plaid_account_ids() -> set[str]:
    return {
        plaid_id
        for plaid_id, row in store.plaid_account_mappings.items()
        if bool(row.get("include_in_household", True))
        and str(row.get("ynab_account") or "").strip()
    }


def excluded_plaid_account_ids() -> set[str]:
    return {
        plaid_id
        for plaid_id, row in store.plaid_account_mappings.items()
        if not bool(row.get("include_in_household", True))
    }
...
async def _db_connect():
    ...
    if psycopg is None:
        raise RuntimeError(
            ...
            "Install psycopg[binary] from requirements.txt."
        )
    return await psycopg.AsyncConnection.connect(DATABASE_URL)


async def initialize_persistence() -> None:
    """Create storage, seed legacy env state once, then load durable state.

    Database state wins over environment seeds on conflict. This makes migration
    from the old PLAID_ACCESS_TOKEN/PLAID_ITEM_ID/PLAID_*_MAP variables safe
    while allowing future connections and mappings to persist automatically.
    """
    if not DATABASE_URL:
        store.persistence_ready = False
    ...
    try:
            ...
            async with conn.cursor(row_factory=dict_row) as cur:
                ...
                await cur.execute(
                    ...
                    CREATE TABLE IF NOT EXISTS plaid_account_mappings (
                        ...
                        ynab_account_alias TEXT,
                        include_in_household BOOLEAN NOT NULL DEFAULT TRUE,
                        notes TEXT NOT NULL DEFAULT '',
                        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
                        updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
                        PRIMARY KEY (household_id, plaid_account_id)
                    )
                    """
                )
                await cur.execute(
                    ...
                    CREATE TABLE IF NOT EXISTS plaid_link_sessions (
                        ...
                        hosted_link_url TEXT NOT NULL DEFAULT '',
                        expiration TEXT NOT NULL DEFAULT '',
                        consumed BOOLEAN NOT NULL DEFAULT FALSE,
                        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
                        updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
                        PRIMARY KEY (household_id, session_ref)
                    )
                    """
                )
                await cur.execute(
                    ...
                    CREATE UNIQUE INDEX IF NOT EXISTS
                        ux_plaid_mapping_household_ynab_active
                    ON plaid_account_mappings (household_id, ynab_account_alias)
                    WHERE include_in_household = TRUE
                      AND ynab_account_alias IS NOT NULL
                    """
                )

                # Seed configured Items only when a row does not already exist.
                for item_id, item in store.plaid_items.items():
                    await cur.execute(
                        """
                        INSERT INTO plaid_items (
                            household_id, item_id, access_token,
                            institution_id, institution_name
                        )
                        VALUES (%s, %s, %s, %s, %s)
                        ON CONFLICT (household_id, item_id) DO NOTHING
                        """,
                        (
                            PLAID_HOUSEHOLD_ID,
                            item_id,
                ...
                # Seed legacy environment mappings only when absent.
                for plaid_id, ynab_alias in PLAID_YNAB_ACCOUNT_MAP.items():
                    await cur.execute(
                        """
                        INSERT INTO plaid_account_mappings (
                            household_id, plaid_account_id, ynab_account_alias,
                            include_in_household
                        )
                        VALUES (%s, %s, %s, TRUE)
                        ON CONFLICT (household_id, plaid_account_id) DO NOTHING
                        """,
                        (PLAID_HOUSEHOLD_ID, str(plaid_id), str(ynab_alias)),
                    )
                ...
                await cur.execute(
                    ...
                    FROM plaid_items
                    WHERE household_id = %s AND active = TRUE
                    ORDER BY item_id
                    """,
                    (PLAID_HOUSEHOLD_ID,),
                )
                ...
                await cur.execute(
                    """
                    SELECT plaid_account_id, ynab_account_alias,
                           include_in_household, notes
        ...
        # Preserve runtime cursors/transaction caches when reloading metadata.
        durable_items: Dict[str, dict] = {}
        for row in item_rows:
            item_id = str(row["item_id"])
            existing = store.plaid_items.get(item_id) or {}
            durable_items[item_id] = {
                "item_id": item_id,
                ...
                "transactions": existing.get("transactions") or {},
            }
        store.plaid_items = durable_items

        store.plaid_account_mappings = {
            str(row["plaid_account_id"]): {
                "plaid_account_id": str(row["plaid_account_id"]),
                "ynab_account": str(row.get("ynab_account_alias") or ""),
                "include_in_household": bool(row.get("include_in_household", True)),
                "notes": str(row.get("notes") or ""),
                "source": "database",
            }
            for row in mapping_rows
        }
        store.persistence_ready = True
        store.persistence_error = None
    except Exception as exc:
        store.persistence_ready = False
        store.persistence_error = f"{type(exc).__name__}: {exc}"
        raise


def hosted_link_exchange_ref(hosted_link_url: str, link_token: str = "") -> str:
    """Return a non-secret reference that can be safely passed back to the API.

    Plaid's Hosted Link URL uses an ``lp...`` slug, while /link/token/get requires
    the actual ``link-...`` token. Keeping those distinct prevents callers from
    accidentally sending the URL slug to Plaid as a link_token.
    """
    url = str(hosted_link_url or "").strip()
    match = re.search(r"/hl/([^/?#]+)", url)
    if match:
    ...
    if link_token:
        ...
        return f"pl:{digest}"
    return f"pl:{uuid.uuid4().hex[:16]}"


async def persist_plaid_link_session(
    session_ref: str,
    link_token: str,
    hosted_link_url: str = "",
    expiration: str = "",
) -> bool:
    """Persist the secret Link token behind a non-secret exchange reference."""
    if not DATABASE_URL:
        return False
    conn = await _db_connect()
    async with conn:
        async with conn.cursor() as cur:
            await cur.execute(
                ...
                INSERT INTO plaid_link_sessions (
                    household_id, session_ref, link_token,
                    hosted_link_url, expiration, consumed, updated_at
                )
                VALUES (%s, %s, %s, %s, %s, FALSE, NOW())
                ON CONFLICT (household_id, session_ref)
                DO UPDATE SET
...
async def resolve_plaid_link_token(identifier: str) -> tuple[str, str]:
    """Resolve either a real Plaid link_token or our non-secret exchange ref."""
    value = str(identifier or "").strip()
    if not value:
        raise HTTPException(status_code=422, detail="Link session reference is empty")

    if value.startswith("link-"):
        return value, ""

    runtime = store.plaid_link_sessions.get(value)
    if runtime and runtime.get("link_token"):
        if bool(runtime.get("consumed")):
            raise HTTPException(
                status_code=409,
                detail="This Hosted Link session has already been exchanged",
            )
        return str(runtime["link_token"]), value
    ...
    if DATABASE_URL:
        ...
        async with conn:
            async with conn.cursor(row_factory=dict_row) as cur:
                await cur.execute(
                    """
                    SELECT session_ref, link_token, consumed
                    FROM plaid_link_sessions
                    WHERE household_id = %s AND session_ref = %s
                    """,
                    (PLAID_HOUSEHOLD_ID, value),
                )
                row = await cur.fetchone()
        if row and row.get("link_token"):
            if bool(row.get("consumed")):
                raise HTTPException(
                    status_code=409,
                    detail="This Hosted Link session has already been exchanged",
                )
            store.plaid_link_sessions[value] = {
                "link_token": str(row["link_token"]),
                "consumed": False,
            }
            return str(row["link_token"]), value

    raise HTTPException(
        status_code=404,
        detail=(
            "Hosted Link session reference not found. Create a new Hosted Link "
            "session and exchange the returned exchange_ref."
        ),
    )


async def mark_plaid_link_session_consumed(session_ref: str) -> None:
    if not session_ref:
        return
    runtime = store.plaid_link_sessions.get(session_ref)
    ...
    async with conn:
        async with conn.cursor() as cur:
            await cur.execute(
                ...
                WHERE household_id = %s AND session_ref = %s
                """,
                (PLAID_HOUSEHOLD_ID, session_ref),
            )


async def persist_plaid_item(item: dict) -> bool:
    """Persist one Plaid Item. Returns False when running in env-only mode."""
    if not DATABASE_URL:
        return False
    conn = await _db_connect()
    async with conn:
        async with conn.cursor() as cur:
            await cur.execute(
                ...
                INSERT INTO plaid_items (
                    household_id, item_id, access_token,
                    institution_id, institution_name, active, updated_at
                )
...
async def persist_plaid_mapping(
    plaid_account_id: str,
    ynab_account_alias: str,
    include_in_household: bool,
    notes: str = "",
) -> bool:
    """Persist an external-account mapping. Returns False in env-only mode."""
    if not DATABASE_URL:
        return False
    conn = await _db_connect()
    async with conn:
        async with conn.cursor() as cur:
            await cur.execute(
                ...
                INSERT INTO plaid_account_mappings (
                    household_id, plaid_account_id, ynab_account_alias,
                    include_in_household, notes, updated_at
                )
                VALUES (%s, %s, %s, %s, %s, NOW())
                ON CONFLICT (household_id, plaid_account_id)
                DO UPDATE SET
                    ynab_account_alias = EXCLUDED.ynab_account_alias,
...
async def load_durable_state_on_startup():
    if DATABASE_URL:
...
class PlaidLinkExchangeRequest(BaseModel):
    link_token: str = Field(
        ...
        min_length=1,
        description=(
            "Hosted Link exchange reference returned by createPlaidHostedLink "
            "(preferred), or a raw Plaid link_token for backwards compatibility"
        ),
    )
    reveal_access_token: bool = Field(
        False,
        description=(
            "If true, return newly created Item access tokens once. This is "
            "normally unnecessary when DATABASE_URL persistence is enabled."
        ),
    )


class PlaidAccountMappingRequest(BaseModel):
    plaid_account_id: str = Field(..., min_length=1)
    ynab_account: Optional[str] = Field(
        None,
        description=(
            "YNAB account alias such as acct:wf_checking_1688. Required when "
            "include_in_household is true."
        ),
    )
    include_in_household: bool = Field(
        True,
        description=(
            "False keeps the external account visible to diagnostics while "
            "excluding it from household reconciliation, e.g. a business account."
        ),
    )
...
async def refresh_plaid_account_balances(force: bool = False) -> Dict[str, dict]:
    ...
    for item_id, item in list(store.plaid_items.items()):
        ...
        for account in data.get("accounts", []):
            ...
            mapping = store.plaid_account_mappings.get(plaid_id) or {}
            included_mapping = effective_plaid_mapping(plaid_id)
            compact[plaid_id] = {
                ...
                "ynab_account": str(mapping.get("ynab_account") or ""),
                "mapped": included_mapping is not None,
                "include_in_household": bool(
                    mapping.get("include_in_household", True)
                ),
...
def plaid_account_alias(plaid_account_id: str) -> str:
    mapping = effective_plaid_mapping(plaid_account_id)
    alias = str((mapping or {}).get("ynab_account") or "")
    if not alias:
        raise HTTPException(
            status_code=422,
            detail=(
                f"Plaid account {plaid_account_id} has no active YNAB mapping. "
                "Use the Plaid mapping endpoint or seed PLAID_YNAB_ACCOUNT_MAP."
            ),
        )
    return alias
...
async def create_plaid_hosted_link(payload: PlaidHostedLinkRequest):
    ...
    hosted_link_url = str(data.get("hosted_link_url") or "")
    expiration = str(data.get("expiration") or "")
    if not link_token or not hosted_link_url:
        raise HTTPException(
            status_code=502,
            detail="Plaid Link creation returned incomplete Hosted Link data",
        )

    exchange_ref = hosted_link_exchange_ref(hosted_link_url, link_token)
    store.plaid_link_sessions[exchange_ref] = {
        "link_token": link_token,
        "hosted_link_url": hosted_link_url,
        "expiration": expiration,
        "consumed": False,
    }
    persisted = await persist_plaid_link_session(
        exchange_ref,
        link_token,
        hosted_link_url,
        expiration,
    ...
    # Do not return the secret Plaid link_token. Callers use exchange_ref; the
    # middleware resolves it server-side.
    return {
...
async def exchange_plaid_hosted_link(payload: PlaidLinkExchangeRequest):
    ...
    Multiple institutions can coexist. When DATABASE_URL is configured, each
    new Item is persisted automatically. Environment JSON remains a supported
    fallback for deployments without durable storage.
    """
    resolved_link_token, session_ref = await resolve_plaid_link_token(
        payload.link_token
    )
    session = await plaid_post(
        "/link/token/get",
        {"link_token": resolved_link_token},
    )
    ...
    for public_token in public_tokens:
        ...
        await hydrate_plaid_item_metadata(item)
        persisted = await persist_plaid_item(item)
    ...
    await mark_plaid_link_session_consumed(session_ref)
    ...
    response: Dict[str, Any] = {
        ...
        "items": added_items,
        "persistence": (
            "database" if DATABASE_URL else "environment_only"
        ),
        "next_step": (
            "Use getPlaidAccounts/getPlaidMappings and setPlaidAccountMapping "
            "to map household accounts."
            if DATABASE_URL
            else
            "Persist every new item_id/access_token pair in PLAID_ITEMS_JSON, "
            "then configure mappings through the API or PLAID_YNAB_ACCOUNT_MAP."
        ),
    }
    if payload.reveal_access_token:
...
def _ynab_account_listing() -> List[Dict[str, Any]]:
    rows: List[Dict[str, Any]] = []
    reverse_mappings: Dict[str, str] = {}
    for plaid_id, mapping in store.plaid_account_mappings.items():
        active = effective_plaid_mapping(plaid_id)
        if active:
            reverse_mappings[str(active.get("ynab_account") or "")] = plaid_id

    for account_id, account in store.accounts.items():
        if account.get("closed") or account.get("deleted"):
            continue
        alias = store.uuid_to_alias.get(account_id) or store.register_alias(
            "acct", account.get("name") or "account", account_id
        )
        rows.append({
            "account": alias,
            "name": account.get("name") or alias,
            "type": account.get("type") or "",
            "on_budget": bool(account.get("on_budget", True)),
            "balance": milli_to_str(int(account.get("balance", 0))),
            "mapped_plaid_account_id": reverse_mappings.get(alias, ""),
        })
    rows.sort(key=lambda row: ((row.get("name") or "").lower(), row["account"]))
    return rows


@app.get(
    "/plaid/mappings",
    summary="Account visibility and Plaid-to-YNAB mapping status",
    operation_id="getPlaidMappings",
)
async def get_plaid_mappings():
    await sync_ynab()
    balances = await refresh_plaid_account_balances(force=True)
    external_accounts: List[Dict[str, Any]] = []

    for plaid_id, account in balances.items():
        mapping = store.plaid_account_mappings.get(plaid_id) or {}
        include = bool(mapping.get("include_in_household", True))
        ynab_alias = str(mapping.get("ynab_account") or "")
        active_mapping = effective_plaid_mapping(plaid_id)
        status_name = (
            "mapped"
            if active_mapping
            else "excluded"
        ...
        external_accounts.append({
            ...
            "status": status_name,
            "ynab_account": ynab_alias,
            "include_in_household": include,
            "mapping_source": str(mapping.get("source") or ""),
            "notes": str(mapping.get("notes") or ""),
        })

    external_accounts.sort(
        key=lambda row: (
            (row.get("institution_name") or "").lower(),
            (row.get("name") or "").lower(),
            row["plaid_account_id"],
    ...
    ynab_accounts = _ynab_account_listing()
    mapped_ynab_aliases = {
        str(row.get("ynab_account") or "")
        for row in external_accounts
        if row.get("status") == "mapped"
    }

    return {
        "persistence": {
            "mode": "database" if DATABASE_URL else "environment_only",
            "ready": store.persistence_ready if DATABASE_URL else False,
            "error": store.persistence_error,
        },
        "counts": {
            "plaid_item_count": len(store.plaid_items),
            "external_account_count": len(external_accounts),
            "mapped_account_count": sum(
                1 for row in external_accounts if row["status"] == "mapped"
            ),
            "excluded_external_account_count": sum(
                1 for row in external_accounts if row["status"] == "excluded"
            ),
            "unmapped_external_account_count": sum(
                1 for row in external_accounts if row["status"] == "unmapped"
            ),
            "ynab_account_count": len(ynab_accounts),
            "ynab_accounts_without_external_mapping": sum(
                1
                for row in ynab_accounts
                if row["account"] not in mapped_ynab_aliases
            ),
        },
...
@app.post(
    "/plaid/mappings",
    summary="Create, update, or exclude a Plaid-to-YNAB account mapping",
    operation_id="setPlaidAccountMapping",
)
async def set_plaid_account_mapping(payload: PlaidAccountMappingRequest):
    await sync_ynab()
    balances = await refresh_plaid_account_balances(force=True)

    plaid_account_id = payload.plaid_account_id.strip()
    if plaid_account_id not in balances:
        raise HTTPException(
            status_code=404,
            detail=f"Plaid account {plaid_account_id} is not currently visible",
        )

    requested_alias = (payload.ynab_account or "").strip()
    canonical_alias = ""

    if payload.include_in_household:
        if not requested_alias:
            raise HTTPException(
                status_code=422,
                detail="ynab_account is required when include_in_household is true",
            )
        ynab_uuid = store.resolve_uuid(requested_alias)
        ynab_account = store.accounts.get(ynab_uuid)
        if not ynab_account or ynab_account.get("closed") or ynab_account.get("deleted"):
            raise HTTPException(
                status_code=404,
                detail=f"YNAB account {requested_alias} was not found or is closed",
            )
        canonical_alias = store.uuid_to_alias.get(ynab_uuid, requested_alias)

        for other_plaid_id, other in store.plaid_account_mappings.items():
            if other_plaid_id == plaid_account_id:
                continue
            other_active = effective_plaid_mapping(other_plaid_id)
            if (
                other_active
                and str(other_active.get("ynab_account") or "") == canonical_alias
            ):
                raise HTTPException(
                    status_code=409,
                    detail=(
                        f"YNAB account {canonical_alias} is already mapped to "
                        f"Plaid account {other_plaid_id}"
                    ),
                )
    elif requested_alias:
        # An excluded account may retain an informational YNAB alias, but if one
        # is provided it still must resolve to a real account.
        ynab_uuid = store.resolve_uuid(requested_alias)
        ynab_account = store.accounts.get(ynab_uuid)
        if not ynab_account or ynab_account.get("closed") or ynab_account.get("deleted"):
            raise HTTPException(
    ...
    row = {
        ...
        "ynab_account": canonical_alias,
        "include_in_household": payload.include_in_household,
        "notes": payload.notes.strip(),
        "source": "database" if DATABASE_URL else "runtime",
    }
    store.plaid_account_mappings[plaid_account_id] = row

    persisted = await persist_plaid_mapping(
        plaid_account_id=plaid_account_id,
        ynab_account_alias=canonical_alias,
        include_in_household=payload.include_in_household,
        notes=payload.notes.strip(),
    )

    # Refresh cached balance metadata so callers immediately see the new state.
    cached = store.plaid_account_balances.get(plaid_account_id)
    if cached is not None:
        cached["ynab_account"] = canonical_alias
        cached["mapped"] = effective_plaid_mapping(plaid_account_id) is not None
        cached["include_in_household"] = payload.include_in_household
        cached["mapping_source"] = row["source"]
    ...
    return {
        **row,
        ...
        "status": (
            "mapped"
            if effective_plaid_mapping(plaid_account_id)
            else "excluded"
            if not payload.include_in_household
            else "unmapped"
        ),
        "note": (
            "Mapping persisted in PostgreSQL."
            if persisted
            else "Mapping is runtime-only; configure DATABASE_URL or mirror it in PLAID_YNAB_ACCOUNT_MAP."
        ),
    }
...
async def sync_plaid_transactions():
    ...
    mapped_account_ids = included_plaid_account_ids()
    excluded_account_ids = excluded_plaid_account_ids()
    mapped_transactions: List[dict] = []
    skipped_unmapped_account_ids = set()
    skipped_excluded_transaction_count = 0
    skipped_unmapped_transaction_count = 0

    for tx in store.plaid_transactions.values():
        plaid_account_id = str(tx.get("account_id") or "")
        if plaid_account_id in excluded_account_ids:
            skipped_excluded_transaction_count += 1
            continue
    ...
    external_transactions = [
        plaid_to_external_transaction(
            tx,
            ynab_account_alias=plaid_account_alias(
    ...
    return {
        "plaid_sync": {
            ...
            "skipped_excluded_transaction_count": skipped_excluded_transaction_count,
            "skipped_unmapped_account_ids": sorted(skipped_unmapped_account_ids),
            "excluded_account_ids": sorted(excluded_account_ids),
...
async def get_middleware_health():
    ...
    mapped_accounts = len(included_plaid_account_ids())
    ...
    persistence_ok = (not DATABASE_URL) or store.persistence_ready
    ...
    checks = {
        ...
        "durable_persistence_configured": bool(DATABASE_URL),
        "durable_persistence_ready": bool(store.persistence_ready),
        "persistence_ready_if_configured": persistence_ok,
    ...
    healthy = all(
        ...
        for name in (
            ...
            "persistence_ready_if_configured",
    ...
    return {
        ...
        "plaid": {
            ...
            "runtime_item_changes_require_manual_persistence": not bool(
                DATABASE_URL and store.persistence_ready
            ),
        },
        "persistence": {
            "mode": "database" if DATABASE_URL else "environment_only",
            "configured": bool(DATABASE_URL),
            "ready": bool(store.persistence_ready),
            "error": store.persistence_error,
            "household_id": PLAID_HOUSEHOLD_ID,
        },
        "mapped_plaid_account_count": mapped_accounts,
        "excluded_plaid_account_count": len(excluded_plaid_account_ids()),
        "primary_checking_balance": balance_diagnostic,
        "reconciliation": {
