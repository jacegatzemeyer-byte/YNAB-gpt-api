...
async def get_current_context():
    ...
    for stx_id, stx in store.scheduled_transactions.items():
        ...
        if cat:
            ...
            obligation = abs(int(stx.get("amount", 0)))
        ...
        upcoming_14d.append({
            ...
            "date": raw_date,
            "account": store.uuid_to_alias.get(stx.get("account_id", ""), stx.get("account_name", "")),
            "payee": stx.get("payee_name") or "Scheduled transaction",
            "amount": milli_to_str(int(stx.get("amount", 0))),
            "category": category_alias(cat_id),
    ...
    return {
        ...
        "upcoming_obligations_14d": sorted(upcoming_14d, key=lambda x: x["date"]),
    }
...
async def get_envelope_status(
    ...
    for cat_id, cat in current_month_categories().items():
        ...
        rows.append({
            ...
            "goal_target": milli_to_str(int(cat.get("goal_target", 0))) if cat.get("goal_target") is not None else None,
            "goal_under_funded": milli_to_str(int(cat.get("goal_under_funded", 0))) if cat.get("goal_under_funded") is not None else None,
    ...
    return {
        ...
        "ready_to_assign": milli_to_str(int(store.current_month_detail.get("to_be_budgeted", 0))),
        "count": len(rows),
        "categories": rows,
    }
...
@app.get(
    "/ynab/triage",
    summary="Actionable transaction triage with payee/import/transfer evidence",
    operation_id="getTriage",
)
async def get_triage(
    format: Literal["csv", "json"] = "json",
    status_filter: Literal["actionable", "unapproved", "uncategorized", "all"] = Query(
        "actionable", alias="status"
    ),
    ...
    for tx_id, tx in store.transactions.items():
        if (tx.get("date") or "") < cutoff:
            continue
        if account_uuid and tx.get("account_id") != account_uuid:
            continue

        is_transfer = tx_is_transfer(tx)
        is_split = bool(tx.get("subtransactions"))
        needs_category = not tx.get("category_id") and not is_transfer and not is_split
        needs_approval = not bool(tx.get("approved"))

        if status_filter == "actionable" and not (needs_category or needs_approval):
            continue
        if status_filter == "unapproved" and not needs_approval:
            continue
        if status_filter == "uncategorized" and not needs_category:
            continue
        ...
        rows.append({
            ...
            "date": tx.get("date"),
            "account": store.uuid_to_alias.get(tx.get("account_id", ""), tx.get("account_name", "")),
    ...
    fieldnames = list(rows[0].keys()) if rows else [
        ...
        "needs_category", "needs_approval", "is_transfer", "transfer_account",
